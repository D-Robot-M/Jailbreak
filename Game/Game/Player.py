...
class Prisoner(Player):
    def __init__(self, x, y):
        Player.__init__(self, x, y,0,0)
        self.aimove_x = [-1, 0, 1]
        self.aimove_y = [-1, 0, 1]
        self.aitime = random.randint(120,180)
        self.aiticker = 0

    def randommovement(self):

        if self.aiticker >= self.aitime:
            self.dx = random.choice(self.aimove_x)
            self.dy = random.choice(self.aimove_y)
            self.aitime = random.randint(120,180)
            self.aiticker = 0
        self.aiticker += 1
